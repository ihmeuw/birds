...
### Purpose: Dementia Forecasting - Test Regression Covs Child
##########################################################################
...
sim_dir <- paste0(save_dir, "FILEPATH")
resid_dir <- paste0(save_dir, "FILEPATH")
...
model_dt <- readr::read_rds(paste0(save_dir, "model_data.rds"))

# RUN MODELS --------------------------------------------------------------

male <- lm(get(paste0("draw_", draw)) ~ as.factor(age_group_id) + get(paste0(cov, "_", draw)) +
           get(paste0("education_", draw)) + region_name,
           data = model_dt[sex_id == 1])

female <- lm(get(paste0("draw_", draw)) ~ as.factor(age_group_id) + get(paste0(cov, "_", draw)) +
             get(paste0("education_", draw)) + region_name,
