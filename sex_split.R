...
sex_split <- function(topic_name, output_dir, bundle_version_id = NULL, data_all_csv = NULL, data_to_split_csv = NULL, 
      ...
      mod1 <- MRBRT(
        ...
        cov_models = list(
          ...
          LinearCovModel(
            ...
            spline_degree = mrbrt_model_age_spline_degree,
  ...
  split_dt[, midage := round(midage/5)*5]
  split_dt[, midage := as.numeric(midage)]
  split_dt[, midyear := floor((year_start + year_end)/2)]
  split_dt[midyear <1990, midyear := 1990]
  split_dt[midage >95, midage := 95]
  
  tosplit_dt <- as.data.table(copy(split_dt))
  
  ## Pull out population data. We need age- sex- location-specific population.
  message("Pulling diabetes population from get_outputs for sex split")
  pop <- get_outputs(age_group_id = c(8:20, 30:32, 235), sex_id = "all", year_id = unique(floor(tosplit_dt$midyear)),
                     location_id=unique(tosplit_dt$location_id), release_id = release_id,
                     cause_id = 587, measure_id = 5, topic = 'cause') 
  
  # Fixing ages
  pop$age_group_name<- substr(pop$age_group_name, 1, 2)
  pop$population<- pop$val
  ...
  if (mrbrt_model_age_cv == TRUE) {
    split_dt <- merge(tosplit_dt4, pred_draws, by = "midage", allow.cartesian = T)
  }
